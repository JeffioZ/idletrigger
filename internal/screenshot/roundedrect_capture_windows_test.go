...
func TestRoundedRectCapture(t *testing.T) {
	...
	for _, scale := range scales {
		for _, language := range []string{"en", "zh-CN"} {
			for _, theme := range []popup.Theme{popup.ThemeLight, popup.ThemeDark} {
				state := fixedSnapshot(language, theme)
				name := "panel-" + language + "-" + themeName(theme) + "-" + scale.name + ".png"
				path := filepath.Join(dir, name)
				err := popup.Capture(state, func(key string) string { return i18n.T(language, key) }, scale.value, func(hwnd windows.Handle) error {
...
func themeName(theme popup.Theme) string {
