// Command trayicons produces purpose-built light and dark taskbar
...
func main() {
	...
	if err := iconfile.WriteICO(filepath.Join(dir, "tray-dark.ico"), sizes, func(size int) image.Image { return iconart.Tray(size, false) }); err != nil {
		fail(err)
	}
	if err := iconfile.WriteICO(filepath.Join(dir, "tray-light.ico"), sizes, func(size int) image.Image { return iconart.Tray(size, true) }); err != nil {
		fail(err)
	}
}
